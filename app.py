...
def get_term():
    if request.method == 'POST':
        ...
        polar = list()
        datum = list()
        for tweet in query_tweets(tt, 100):
            try:
                txt = tweet.text
    ...
    return render_template('mp.html', dataframe=dff.to_html(index=False), res=dffres.to_html(index=False) , twww=dftwit.to_html(index=False))
